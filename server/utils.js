...
class utils {
    ...
    static async startDB() {
        try {
            ...
            if (!u) {
                ...
                newUser.setPassword(process.env.ADMIN_DEFAULT || 'socketlover');
                await newUser.save();
            }
        } catch (e) {
        }
    ...
    static async getUserInfo(find_id) {
        ...
        //console.log(ris);
        try {
            ris.user = await user.getModel().findOne({
                _id: find_id
            }).select('_id nickname win lose admin eliminated').exec();
            if (!ris.user) {
                return ris;
            }
            ris.isOnline = socket.checkIfExist(find_id);
            ris.room = await room.getModel().findOne({
                host_id: find_id
            }).exec();

            ris.game = await game.getModel().findOne({
                $or: [{
                    'player1_id': find_id
                }, {
                    'player2_id': find_id
