...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  init() {
    ...
    if (this.game) {
      this.subscribers.push(
        this._gs.enemyID.subscribe((_id) => {
          this.subscribers.push(
            this._gs.nickname.subscribe((_n) => {
      ...
      this.subscribers.push(
        this._cs.getUsers().subscribe((data) => {
      ...
      this.subscribers.push(
        this._ss.onStatusChange().subscribe((data) => {
          let trovato = false;
    ...
    this.subscribers.push(
      this._ss.onNewMessage().subscribe((data: any) => {
        ...
        if (
          !this.game &&
          (this.list ||
            (this.chat.id !== data.from && data.from !== this._us.getId()))
        ) {
          let text: string = data.text;
          text = text.length < 20 ? text : text.slice(0, 20).concat(" . . .");
          this._toastr.info(text, "Messaggio da " + data.from_name);
        }
      })
    );
  }
  ...
  loadChat(user) {
    ...
    this.subscribers.push(
      this._cs.getChatFrom(user.id).subscribe((data) => {
  ...
  send(text) {
    ...
    this.subscribers.push(
      this._cs.send(text, this.chat.id).subscribe(
        (data) => {},
        (err) => {
          this._toastr.error(err.error.message, "Errore invio");
        }
      )
    );
  }
  ...
  search(nickname: string) {
    ...
    this.contacts = this.contacts.filter(
      (c) => c.nickname.toLowerCase().search(nickname) !== -1
    );
  }

  info(user) {
    this.subscribers.push(
      this._us.getStatus(user.id).subscribe((data) => {
