...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  loadUser() {
    this.subscribers.push(this._us.getAllUsers().subscribe(
      (data) => {
      ...
      (err) => {
        this.toastr.errorToastr(err.error.message, 'Errore richiesta');
      }
    ));
  }
  ...
  pagination(val) {
    ...
    this.usersToShow = this.users.slice( this.currentPage * this.itemsPerPage, (this.currentPage + 1) * this.itemsPerPage);
  ...
  evolveInAdmin() {
    this.subscribers.push(this._us.evolveUser(this.userToEvolve.id).subscribe(
      (data) => {
        this.toastr.infoToastr('È un amministratore.', this.userToEvolve.nickname);
        this.loadUser();
      },
      (err) => {
  ...
  deleteUser(reason: string) {
    ...
    this.subscribers.push(this._us.deleteUser(this.userToRemove.id, reason).subscribe(
      (data) => {
        this.toastr.infoToastr('È stato eliminato.', this.userToRemove.nickname);
        this.loadUser();
      },
      (err) => {
        this.toastr.errorToastr(err.error.message, 'Errore.');
