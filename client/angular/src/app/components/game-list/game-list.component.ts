...
export class GameListComponent implements OnInit, OnDestroy {
  ...
  init() {
    if (this._us.getToken()) {
      ...
      this.subscribers.push(
        this._gs.getHostedMatches().subscribe(
          (data) => {
      ...
      this.subscribers.push(
        this._ss.onRoomsUpdate().subscribe((obj) => {
      ...
      // alla mia partita
      this._ss.onJoinGame().subscribe((obj) => {
  ...
  pagination(val) {
    ...
    this.roomsToShow = this.rooms.slice(
